...
contract Timelock is ITimelock {
    ...
    event SignalRedeemUsdb(address vault, address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        IVault(_vault).setUsdbAmount(_token, _usdbAmount);
    }
    ...
    function setUsdbAmounts(address _vault, address[] memory _tokens, uint256[] memory _usdbAmounts) external onlyKeeperAndAbove {
        for (uint256 i = 0; i < _tokens.length; i++) {
            IVault(_vault).setUsdbAmount(_tokens[i], _usdbAmounts[i]);
        }
    }

    function updateUsdbSupply(uint256 usdbAmount) external onlyKeeperAndAbove {
        address usdb = IBlpManager(blpManager).usdb();
        uint256 balance = IERC20(usdb).balanceOf(blpManager);

        IUSDB(usdb).addVault(address(this));

        if (usdbAmount > balance) {
            uint256 mintAmount = usdbAmount.sub(balance);
            IUSDB(usdb).mint(blpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdbAmount);
            IUSDB(usdb).burn(blpManager, burnAmount);
        }

        IUSDB(usdb).removeVault(address(this));
    }
    ...
    function signalRedeemUsdb(address _vault, address _token, uint256 _amount) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemUsdb", _vault, _token, _amount));
        _setPendingAction(action);
        emit SignalRedeemUsdb(_vault, _token, _amount);
    }
    ...
    function redeemUsdb(address _vault, address _token, uint256 _amount) external onlyAdmin {
        ...
        address usdb = IVault(_vault).usdb();
        IVault(_vault).setManager(address(this), true);
        IUSDB(usdb).addVault(address(this));

        IUSDB(usdb).mint(address(this), _amount);
        IERC20(usdb).transfer(address(_vault), _amount);

        IVault(_vault).sellUSDB(_token, mintReceiver);
