...
contract Reader is Governable {
    ...
    function getMaxAmountIn(
        ...
        uint256 maxUsdbAmount = _vault.maxUsdbAmounts(_tokenIn);

        if (maxUsdbAmount != 0) {
            if (maxUsdbAmount < _vault.usdbAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdbAmount.sub(_vault.usdbAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10**tokenInDecimals).div(10**USDB_DECIMALS);
    ...
    function getAmountOut(
        ...
        {
            uint256 usdbAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdbAmount = usdbAmount.mul(10**USDB_DECIMALS).div(10**tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdbAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdbAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.usdbAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usdbAmount);
    ...
    function getFullVaultTokenInfo(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxUsdbAmounts(token);
