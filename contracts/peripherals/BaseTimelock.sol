...
import "./interfaces/IBaseTimelock.sol";
...
contract BaseTimelock is IBaseTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "BaseTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "BaseTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "BaseTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "BaseTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        require(_target != address(this), "BaseTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "BaseTimelock: invalid _buffer");
        require(_buffer > buffer, "BaseTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }
    ...
    function _mint(
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "BaseTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "BaseTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "BaseTimelock: action time not yet passed");
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "BaseTimelock: invalid _action");
