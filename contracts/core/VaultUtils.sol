...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getBuyUsdbFeeBasisPoints(address _token, uint256 _usdbAmount) public view override returns (uint256) {
        return getFeeBasisPoints(_token, _usdbAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), true);
    }

    function getSellUsdbFeeBasisPoints(address _token, uint256 _usdbAmount) public view override returns (uint256) {
        return getFeeBasisPoints(_token, _usdbAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), false);
    }
    ...
    function getSwapFeeBasisPoints(
        ...
        uint256 feesBasisPoints0 = getFeeBasisPoints(_tokenIn, _usdbAmount, baseBps, taxBps, true);
        uint256 feesBasisPoints1 = getFeeBasisPoints(_tokenOut, _usdbAmount, baseBps, taxBps, false);
    ...
    function getFeeBasisPoints(
        ...
        uint256 initialAmount = vault.usdbAmounts(_token);
        uint256 nextAmount = initialAmount.add(_usdbDelta);
        if (!_increment) {
            nextAmount = _usdbDelta > initialAmount ? 0 : initialAmount.sub(_usdbDelta);
        }

        uint256 targetAmount = vault.getTargetUsdbAmount(_token);
        if (targetAmount == 0) {
            return _feeBasisPoints;
        }
